  ...
  return (
    <main className="onboarding-shell">
      ...
      <section className="onboarding-card">
        ...
        {step === 5 && <FinishStep api={api} config={config} onChange={onChange} />}
...
function FinishStep({ api, config, onChange }: { api: DesktopApi; config: AppConfig; onChange(snapshot: DesktopSnapshot): void }) {
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
